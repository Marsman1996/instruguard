...
def fix_asm(asm_file, instru_info, output):
    ...
    list_optins = dict_ins["list_optins"]
    ...
    for line in lines:
        ...
        if line[0] == "\t" and line[1] == ".":
            if (
                line.find("text\n") == 2
                or line.find("section\t.text") == 2
                or line.find("section\t__TEXT,__text") == 2
                or line.find("section __TEXT,__text") == 2
            ):
                instr_ok = 1
                dump_to_file(afo, temp_line_list)
        ...
        if line[0:3] == ".LC":
            ...
            if len(list_ins) > 0 and prefix in list_ins[0][0:3]:
                if len(list_optins) > 0 and prefix == list_optins[0][1][0]:
                    temp_line_list[-1] = (
                        "# " + temp_line_list[-1].rstrip("\n") + " # OptIns del\n"
                    )
                    list_optins.pop(0)
                elif prefix == list_ins[0][0]:
                    ...
                    if _prev_reg.find("(%") >= 0:
                        ...
                        temp_line_list.insert(
                            -1, "\tmovq __afl_prev_loc(%%rip), %s\n" % (_prev_reg)
                        )
            ...
            # match MIL
                ...
                MIL_num += 1
                if len(list_optins) > 0 and prefix == list_optins[0][0]:
                    temp_line_list.insert(
                        -1,
                        "\tmovq $%s, __afl_prev_loc(%%rip)\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--asm_file", help="Assembly code file.", required=True)
    parser.add_argument(
        "--instru_info",
        help="Json file which contains the instrumentation infomation.",
        required=True,
    )
    parser.add_argument(
