...
static void find_as(u8* argv0) {
  ...
  if (afl_path) {
    ...
    tmp = alloc_printf("%s/instruguard/as-ig", afl_path);

    if (!access(tmp, X_OK)) {
      as_path = alloc_printf("%s/instruguard", afl_path);
      ck_free(tmp);
      return;
    }
  ...
  if (slash) {
    ...
    *slash = '/';
    ...
    tmp = alloc_printf("%s/instruguard/afl-as-ig", dir);

    if (!access(tmp, X_OK)) {
      as_path = alloc_printf("%s/instruguard", dir);
      ck_free(tmp);
      return;
    }
  ...
  if (!access(AFL_PATH "/instruguard/as-ig", X_OK)) {
    as_path = alloc_printf("%s/instruguard", AFL_PATH);
    return;
  }

  FATAL("Unable to find AFL wrapper binary for 'as-ig'. Please set AFL_PATH");
...
int main(int argc, char** argv) {
  ...
  if (isatty(2) && !getenv("AFL_QUIET")) {
    ...
    SAYF(cCYA "afl-ig " cBRI VERSION cRST " by <lcamtuf@google.com>\n");
